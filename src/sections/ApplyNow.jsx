...
import coursesData from '../constants/course.json';
        ...
        <div>
          ...
          >
            ...
            {Array.isArray(countries) && countries.map((c, idx) => (
              <option key={idx} value={c.nationality}>{c.nationality}</option>
            ))}
          </select>
...
function StepEducation({ data, onChange, errors }) {
  // Flatten all courses from bachelor and master
  const courseOptions = [
    ...(coursesData?.degrees?.bachelor || []),
    ...(coursesData?.degrees?.master || [])
  ];
  return (
    <div>
        ...
        <div>
          ...
          >
            ...
            {courseOptions.map((c, idx) => (
              <option key={idx} value={c.name}>{c.name} ({c.abbreviation}) - {c.field}</option>
            ))}
          </select>
